...
## load in the file of bridge speed cases
id.file<-fread('/scratch/WGS10K/data/manifest/sample/include_samples_latest.txt',header=FALSE)
setnames(id.file,c('Illumina.ID','id1','id2','cohort','notes'))
id.file<-subset(id.file,cohort=='SPEED')
...
genes<-cbind(se.t.types,sapply(all.se.ft,function(x) paste(unique(sprintf("%s:%s-%s:%s(%s)",x$CANVAS_CHROM,x$CANVAS_START,x$CANVAS_END,x$name,x$BRIDGE_ID)),collapse=',')))
genes<-cbind(se.t.types,sapply(all.se.ft,function(x) paste(unique(sprintf("%s(%s)",x$name,x$BRIDGE_ID)),collapse=',')))
table(unlist(strsplit(genes[,2],",")))
## all genes
gene_peeps<-unique(unlist(strsplit(genes[,2],",")))
gper<-data.table(gene=sub("^([^\\(]+).*","\\1",gene_peeps),person=sub("^[^\\(]+\\(([^\\)]+)\\)","\\1",gene_peeps))
## F009946 - looks dodgy
gper<-gper[gper$person!='F009946',]
peeps<-table(gper$person)
gper<-gper[gper$person %in% names(peeps[peeps<3]),]
## for lab talk let's look at the CD19 in more detail

non.cpm<-do.call('rbind',all.se.ft)
cd19<-subset(non.cpm,name=='CD19')
## chicp suggests this is a gene deletion event and SE unlikely to be main issue. Hemizygous 
cor01a<-subset(non.cpm,name=='CORO1A')
## chicp suggests this is a gene deletion event and SE unlikely to be main issue. Hemizygous
il7r<-subset(non.cpm,name=='IL7R')
## over 4mb away from gene - SE in CD4+ CD25- IL17 PMA stim cells - PCHi-C is Total_CD4 Activated  Links to FYB - much closer perhaps a candidate gene ? Hemizygous
tbx1<-subset(non.cpm,name=='TBX1')
## chicp suggests gene deletion
itk<-subset(non.cpm,name=='ITK')
## hemizygous deletion of TNFAIP8 TSS - ITK is 4-5Mb away
hax1<-subset(non.cpm,name=='HAX1')
## hemizygous far away links but megakaytocytes UBE2Q1
map3k14<-subset(non.cpm,name=='MAP3K14')
## possible poor quality ? Additional T cell specific interactions with FMNL1 which is closer
dock8<-subset(non.cpm,name=='DOCK8')
## dock8 gene deletion
fpr1<-subset(non.cpm,name=='FPR1')
## deletion of SIGLEC9 links back to FPR1 also - distance approx 700Kb
