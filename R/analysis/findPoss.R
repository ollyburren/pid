...
ws<-lapply(names(all),function(n){
    ...
    tmp<-unique(tmp[,.(ensg,name,ftype,p.bridge_id,p.seqnames,p.gt,gt,GNOMAD_AF,WGS10K_AF,Existing_variation,Consequence,CADD_PHRED)])
    ...
    tmp$p.gt<-NULL
    tmp$sv.type<-n
    setnames(tmp,c('ensg','gene','feat.type','individual','chr','snp.gt','gnomad_af','wgs10k_af','exist.var','consequence','cadd','pid','sv.gt','sv.type'))
    setcolorder(tmp,c('sv.type','ensg','gene','feat.type','individual','chr','sv.gt','snp.gt','gnomad_af','wgs10k_af','exist.var','consequence','cadd','pid'))
    tmp$poss.diag<-tmp$individual %in% poss.diag
    tmp$def.diag<-tmp$individual %in% def.diag
    ##fix numeric columns
    for(n in c('chr','snp.gt','gnomad_af','wgs10k_af','cadd')){
