...
cvid.dir<-'/home/ob219/scratch/cvid/rafal_results'
...
PMI.OUT.DIR<-'/home/ob219/scratch/cvid/pmi'
GENE.SCORE.OUT.DIR<-'/home/ob219/scratch/cvid/gene_score'
...
processFile<-function(DT,ncases=377,ncontrols=6113){
  #DT<-subset(DT,hwe>hwe.thresh & maf>maf.thresh & info_impute > info.impute.thresh)
  setnames(DT,c('chr','rsid','pos','allele_a','test','NMISS','or','stat','p','chi.sq','p.adj'))
  ...
  DT.maf[,ppi:=approx.bf.p(p.adj,maf,N,ncases/N,1e-4),by=ld]
  ...
  options(scipen=999)
  pmi.o.file<-file.path(PMI.OUT.DIR,paste('CVID','pmi',sep='.'))
